...
pub fn derive(label: u8, master_key: [aes.Aes128.block.block_length]u8, master_salt: []const u8, comptime out_size: usize, out: *[out_size]u8) !void {
    var iv: [IV_LENGTH]u8 = @splat(0);
    @memcpy(iv[0..master_salt.len], master_salt);
    iv[7] ^= label;

    const ctx = aes.Aes128.initEnc(master_key);
    const input: [out_size]u8 = @splat(0);
    ctr(std.crypto.core.aes.AesEncryptCtx(aes.Aes128), ctx, out, &input, iv, .big);
}
...
test "key derivation" {
    ...
    const assertDerivedKey = struct {
        pub fn assertDerivedKey(T: type, comptime expected_value: T, label: u8) !void {
            ...
            try derive(label, master, &salt, bytes, &out);
    ...
    try assertDerivedKey(u128, expected_rtp_enc_key, Labels.RTP_ENC_KEY_LABEL);
    try assertDerivedKey(u128, expected_rtp_auth_key, Labels.RTP_AUTH_LABEL);
    try assertDerivedKey(u112, expected_rtp_salt, Labels.RTP_SALT_LABEL);
}
